...
const AddGo09 = () => {
    ...
    const saveData = async (e) => {
        ...
        e.preventDefault();
        let pumpData = await fetch(`${process.env.REACT_APP_BACKEND_URL_KEY}/joiya/go09/data/`, {
            method: 'POST',
            body: JSON.stringify({
    ...
    const userData = async () => {
        let res = await fetch(`${process.env.REACT_APP_BACKEND_URL_KEY}/joiya/go09/data`)
        res = await res.json();
