...
const EditAlWali = () => {
    ...
    const getUserDetail = async () => {
        let res = await fetch(`${process.env.REACT_APP_BACKEND_URL_KEY}/joiya/alwali/data/${params.id}`)
        res = await res.json();
    ...
    return (
        <>
            <div className="form-container card border-0 shadow rounded-3 my-5 p-5">
                <h1 className="mb-5">AL Wali Power Station</h1>
                <hr />
                <form>
                    {/* sup 1 */}
