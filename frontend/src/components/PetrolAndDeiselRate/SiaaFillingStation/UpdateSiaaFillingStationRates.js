...
const UpdateSiaaFillingStationRates = () => {
  ...
  const getUserDetail = async () => {
    let res = await fetch(`${process.env.REACT_APP_BACKEND_URL_KEY}/joiya/rate/siaaFillingStation/${params.id}`)
    res = await res.json();
  ...
  const updateData = async (e) => {
    ...
    let res = await fetch(`${process.env.REACT_APP_BACKEND_URL_KEY}/joiya/rate/siaaFillingStation/${params.id}`, {
      method: "put",
      body: JSON.stringify({ sup_purchase_rate, sup_sale_rate }),
      headers: {
...
export default UpdateSiaaFillingStationRates;
